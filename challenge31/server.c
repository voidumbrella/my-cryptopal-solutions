 ...
 *
 * I don't want to spend time writing a web server just for this,
 * so this just uses a simple protocol to communicate over a socket.
 *
 * The server simply reads packets that look like:
 *
 * [length][file][SHA1 HMAC]
 * |       |     |_ 20 bytes
 * 32 bit  |
 *         |_ buffer with given length
 *
 * and writes 1 if the HMAC is valid, and 0 otherwise.
 */
